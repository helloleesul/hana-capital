<template>
  <validation-observer ref="observer" v-slot="{ handleSubmit }" v-if="show">
    ...
    >
      ...
      >
        <b-form-group
          id="newPassword2-input-group"
          label="새 비밀번호 확인"
          label-for="newPassword2-input"
          class="mt-3"
        >
          <b-form-input
            ...
            placeholder="비밀번호를 입력하세요."
          ></b-form-input>
          <b-form-invalid-feedback id="newPassword2-input-feedback">{{
            validationContext.errors[0]
          }}</b-form-invalid-feedback>
        </b-form-group>
      </validation-provider>
  ...
  methods: {
    async changePwd() {
      ...
      if (data.code === "0000") {
        ...
        this.$bvModal
          .msgBoxOk("입력하신 현재 비밀번호가 일치하지 않습니다.", {
            title: "비밀번호 불일치",
            size: "sm",
            buttonSize: "sm",
            okVariant: "danger",
